function [lat,lon] = polyjoinOS(latcells,loncells)
% polyjoinOS: Convert line or polygon parts from cell arrays to vector form
% Open-source version of function polyjoin
% 
%   [LAT,LON] = polyjoinOS(LATCELLS,LONCELLS) converts polygons from cell
...
%     [lat,lon] = polyjoinOS(latcells,loncells);
%     [lat lon]
...
if isempty(latcells) && isempty(loncells)
    ...
    assert(isequal(size(latcells),size(loncells)), ...
        'map:polyjoinOS:cellvectorSizeMismatch', ...
    ...
    assert(isequal(latSizes,lonSizes), ...
        'map:polyjoinOS:cellContentSizeMismatch', ...
